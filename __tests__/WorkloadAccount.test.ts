...
describe('types.ts', () => {
  it('creates WorkloadAccount customer correctly from lower', async () => {
    ...
    expect(WorkloadAccount.getCustomerName(customerId, orgUnitName)).toBe("ProjectnameDev");
  })
  ...
  it('creates WorkloadAccount description correctly lower', async () => {
    const customerId = 'projectname';
    const orgUnitName = 'dev';
    expect(WorkloadAccount.getDescription(customerId, orgUnitName)).toBe(`The Projectname Dev Account`);
  })
  ...
  it('creates WorkloadAccount email correctly lower', async () => {
    ...
    const customerId = 'projectname';
    const orgUnitName = 'dev';
    expect(WorkloadAccount.getEmail(email, customerId, orgUnitName)).toBe(`user+projectname-dev@example.com`);
  })
